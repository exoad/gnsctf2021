...
def hamming_score_bytes(text1: bytes, text2: bytes) -> float:
    return hamming_distance_bytes(text1,
                                  text2) / (8 * min(len(text1), len(text2)))
...
def compute_key_length(text: bytes) -> int:
    ...
    for klen in range(2, math.ceil(len(text) / 2)):
        ...
        chunks = [text[i:i + klen] for i in range(0, len(text), klen)]
...
print(
    compute_key_length(
        b"0901073031250910075d300740061600562b055c0d463a131e5009"))
