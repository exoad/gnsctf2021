...
def decrypt():
    ...
    def finddistance(input1, input2):  # Hemming Distance
        ...
        input1 = ''.join(['{:08b}'.format(ord(i), 'b') for i in input1
                          ])  ## 'b' in (ord(i),'b') is redundant for {:08b}
        input2 = ''.join(
            ['{:08b}'.format(ord(i), 'b') for i in input2]
        )  ## {:08b} means to make a string that is 8 chars long, ascii to bytes
        ## int converts binary string to base 10 number(long) for xor'ing, format converts result to binary, then lists bits.
        try:
            result = list(
                format(int(input1, base=2) ^ int(input2, base=2), 'b'))
        ...
        for i in range(0, 100):
            try:
                d.append(
                    float(
                        finddistance(
                            encryptedmessage[size * i:size * (i + 1)],
                            encryptedmessage[size * (i + 1):size * (i + 2)])))
    ...
    for size in probablekeysizes:
        ...
        blockedmessage = [
            encryptedmessage[i:i + size]
            for i in range(0, len(encryptedmessage), size)
        ]
        ...
        while j < size:
            ...
            for block in blockedmessage:
                try:
                    ...
                    if errorraised == 0:
                        print("index error at index " + str(
                            blockedmessage.index(block)) + " out of " + str(
                                len(blockedmessage) - 1))
                        errorraised = 1
        ...
        for block in newblockedmessage:
            ...
            for i in range(127):
                ...
                if tempscore >= score:
                    ...
                    temprecord = [
                        chr(i),
                        newblockedmessage.index(block), tempscore
                    ]
                    for c in newstring:
        ...
        for letter in keychars[size]:
            print("Letter " + letter[0] + " at position " + str(
                letter[1]) + " with score " + str(letter[2]))
            averagescore += letter[2]
        averagescore = averagescore / size
        if allprintable >= len(
                encryptedmessage
        ):  ## Advantage to keys that only produce printable characters
    ...
    themax = max([keychars[i][i] for i in keychars.iterkeys()
                  ])  ## get highest average score among keysizes
    for i in keychars.iterkeys():
