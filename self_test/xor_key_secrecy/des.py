...
def single_key_xor(hex_string, char_key):
    ...
    for i in range(0, len(hex_string) - 1, 2):
        decryption += chr(
            int(hex_xor(hex_string[i] + hex_string[i + 1], hex(ord(char_key))),
                16))
    return decryption
...
def ctf5():
    ...
    all_points = sorted(all_points.items(),
                        key=lambda x: x[1][0],
                        reverse=True)
    # Print top 5 items
    print(all_points[:5])
...
def ctf6():
    ...
    # Cipher Text
    ct = '0901073031250910075d300740061600562b055c0d463a131e5009'
    key = 'repeating key xor'
